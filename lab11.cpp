...
int main()
{
	...
	int size = rand() % 5 + 3; 
	int* a = NULL, * t = NULL; // a и t указатели
	a = new int[size];  /*Выделение памяти осуществляется с помощью оператора new и имеет вид :
	...
	cin >> del;
	bool flag; /*Он используется для проверки логических условий или как флаг состояний, 
	как выключатель - ВКЛ и ВЫКЛ (true и false)*/
	flag = check(del, size);
	if (flag)
	{
		t = new int[size];
